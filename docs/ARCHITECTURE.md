...
| `queue/` | `QueueService` (durable queue + state machine, SPEC §13), `QueueWorker` (polling loop, concurrency limits via DB-level capacity check, workspace locks, **stale-job reaper** — scans every 60s for stuck running/preparing jobs), `runner` (job lifecycle: worktree → OCR process → parse → findings, with exception safety net + prepare timeout), `recovery` (interrupted jobs + orphan worktrees at startup), `bus` (in-process SSE fan-out) |
