...
| 10 | Every supported OCR review flag represented or deliberately controlled | `test_ocr_adapter.py::test_build_range_command`, `::test_build_commit_command`, `::test_build_workspace_command`, `::test_additional_arguments_appended_before_forced`; profile schema covers all SPEC §8 controls; `format=json`/`audience=human` are runner-forced so results stay structured while live progress is visible |
